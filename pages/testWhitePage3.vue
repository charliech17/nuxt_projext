<template>
    <div>   
        <div style="background-color: aqua;">
            <h1>Page 2 ~~~</h1>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Iusto illum ratione culpa suscipit sunt. Dolores, rem error reiciendis suscipit ea nam saepe aut consequatur harum id, est, repudiandae obcaecati vel?
            Accusantium, voluptatum ullam! Quis earum harum illum nostrum nam beatae odit maxime illo quo natus suscipit culpa molestias hic adipisci repellendus sed voluptatum, reiciendis possimus deserunt ipsum consectetur! Quae, voluptatum!
            Tempore sint, in dolorum aut ab amet et. Reprehenderit animi est vel? Assumenda error, architecto natus dolore culpa aliquam, harum voluptas magnam nesciunt placeat, doloremque dolorem ab debitis quia velit?
            Vero nihil sint, sed reprehenderit dicta suscipit tenetur facilis, maiores magnam blanditiis qui, molestiae deserunt nam. Minus aliquid hic quam ipsam error et sint, dolores ipsa minima natus totam. Blanditiis?
            Placeat illo esse, natus ut ea porro quaerat, possimus deleniti nisi temporibus autem cumque veritatis asperiores commodi quod ipsum dolores sapiente minus aperiam. Commodi neque libero dolor illo perspiciatis sint.
            Nostrum rerum magnam, impedit nobis asperiores aperiam, sed perferendis nisi dolores minus a architecto vel culpa quo libero facilis adipisci doloremque dolore! Fugiat magni, earum blanditiis pariatur labore quas unde.
            Et dolor aliquid mollitia magnam quae recusandae quidem ipsam pariatur eveniet. Velit totam at quibusdam eaque sequi facilis. Provident voluptates quis voluptas odio quibusdam similique iusto nemo neque aut eius!
            Vero eum et mollitia sunt in delectus ab dolore, fugit quae laborum nobis quisquam quibusdam quas velit, similique iste totam sequi tenetur voluptas commodi. Molestias unde recusandae earum eum saepe!
            Beatae, ex tempore inventore ab quis dolorum optio laborum atque labore ad nobis ullam esse saepe quas vel molestias sunt corrupti maiores minus. Omnis enim repellat unde. Natus, fugit recusandae.
            Atque eligendi quae voluptates deserunt nihil delectus odit libero commodi, dolorem debitis, consectetur ipsum ea quo obcaecati vero ut, iste suscipit quas minus! Sint quia asperiores laboriosam earum fuga nam!
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati aliquid eos eius maiores exercitationem quos atque quibusdam odio nobis voluptate illo doloribus asperiores fugiat, quas est, architecto velit sed quae.
            Sequi fuga alias perferendis accusantium, ipsam adipisci necessitatibus odit, vero, eaque obcaecati dolores! Molestiae, nesciunt explicabo! Nesciunt ad similique, nisi asperiores ipsum magni hic, labore fugit dolorum necessitatibus soluta laudantium?
            Temporibus, nam. Sunt sint cumque vitae totam eligendi quis adipisci doloremque ratione vero veniam tenetur nesciunt maiores sequi beatae qui quo quia, ut eius dignissimos aut natus dolorem id! Eos?
            Vel nihil, rerum ducimus consequuntur debitis tenetur cumque amet necessitatibus eos nesciunt quaerat molestias omnis voluptatem illum eveniet impedit rem cum? Accusantium ad nostrum omnis aspernatur pariatur non cum vitae?
            Velit quis earum dolor obcaecati fuga repellat assumenda nisi fugiat? Repudiandae, libero quae fuga itaque sed inventore consequatur incidunt laudantium animi tempore, necessitatibus consequuntur omnis laboriosam perspiciatis saepe quia culpa?
            Labore, repellendus totam, veritatis asperiores minima consequatur soluta in impedit libero itaque quasi incidunt similique ipsa exercitationem earum corrupti officia neque, eius sint sed maiores illum quos. Incidunt, ut cum?
            Illo eius nostrum veritatis iste voluptatum dolores corporis deleniti. Libero, ab qui consequuntur quam fuga placeat, repellat nemo illo officia culpa eum incidunt dolor vel veniam suscipit harum velit saepe.
            Quisquam architecto quod nobis maiores, doloremque non, consequuntur molestias iste incidunt officiis facere ipsa aperiam veritatis fugiat autem illo qui est at eius tempore, nostrum soluta facilis inventore. Perspiciatis, ad?
            Ipsam perspiciatis dolor voluptas neque, nihil quos, eos consectetur cumque iure quia, sunt quibusdam earum illum deserunt voluptates alias porro quae corrupti! Rem consequuntur, impedit incidunt magni expedita recusandae error.
            Sunt, eligendi blanditiis laudantium quaerat nobis doloribus accusamus, ratione officia, possimus voluptate nihil eius hic repellat laboriosam eveniet nostrum! Sit et quidem quia maiores exercitationem commodi quas voluptates quos atque.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati aliquid eos eius maiores exercitationem quos atque quibusdam odio nobis voluptate illo doloribus asperiores fugiat, quas est, architecto velit sed quae.
            Sequi fuga alias perferendis accusantium, ipsam adipisci necessitatibus odit, vero, eaque obcaecati dolores! Molestiae, nesciunt explicabo! Nesciunt ad similique, nisi asperiores ipsum magni hic, labore fugit dolorum necessitatibus soluta laudantium?
            Temporibus, nam. Sunt sint cumque vitae totam eligendi quis adipisci doloremque ratione vero veniam tenetur nesciunt maiores sequi beatae qui quo quia, ut eius dignissimos aut natus dolorem id! Eos?
            Vel nihil, rerum ducimus consequuntur debitis tenetur cumque amet necessitatibus eos nesciunt quaerat molestias omnis voluptatem illum eveniet impedit rem cum? Accusantium ad nostrum omnis aspernatur pariatur non cum vitae?
            Velit quis earum dolor obcaecati fuga repellat assumenda nisi fugiat? Repudiandae, libero quae fuga itaque sed inventore consequatur incidunt laudantium animi tempore, necessitatibus consequuntur omnis laboriosam perspiciatis saepe quia culpa?
            Labore, repellendus totam, veritatis asperiores minima consequatur soluta in impedit libero itaque quasi incidunt similique ipsa exercitationem earum corrupti officia neque, eius sint sed maiores illum quos. Incidunt, ut cum?
            Illo eius nostrum veritatis iste voluptatum dolores corporis deleniti. Libero, ab qui consequuntur quam fuga placeat, repellat nemo illo officia culpa eum incidunt dolor vel veniam suscipit harum velit saepe.
            Quisquam architecto quod nobis maiores, doloremque non, consequuntur molestias iste incidunt officiis facere ipsa aperiam veritatis fugiat autem illo qui est at eius tempore, nostrum soluta facilis inventore. Perspiciatis, ad?
            Ipsam perspiciatis dolor voluptas neque, nihil quos, eos consectetur cumque iure quia, sunt quibusdam earum illum deserunt voluptates alias porro quae corrupti! Rem consequuntur, impedit incidunt magni expedita recusandae error.
            Sunt, eligendi blanditiis laudantium quaerat nobis doloribus accusamus, ratione officia, possimus voluptate nihil eius hic repellat laboriosam eveniet nostrum! Sit et quidem quia maiores exercitationem commodi quas voluptates quos atque.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati aliquid eos eius maiores exercitationem quos atque quibusdam odio nobis voluptate illo doloribus asperiores fugiat, quas est, architecto velit sed quae.
            Sequi fuga alias perferendis accusantium, ipsam adipisci necessitatibus odit, vero, eaque obcaecati dolores! Molestiae, nesciunt explicabo! Nesciunt ad similique, nisi asperiores ipsum magni hic, labore fugit dolorum necessitatibus soluta laudantium?
            Temporibus, nam. Sunt sint cumque vitae totam eligendi quis adipisci doloremque ratione vero veniam tenetur nesciunt maiores sequi beatae qui quo quia, ut eius dignissimos aut natus dolorem id! Eos?
            Vel nihil, rerum ducimus consequuntur debitis tenetur cumque amet necessitatibus eos nesciunt quaerat molestias omnis voluptatem illum eveniet impedit rem cum? Accusantium ad nostrum omnis aspernatur pariatur non cum vitae?
            Velit quis earum dolor obcaecati fuga repellat assumenda nisi fugiat? Repudiandae, libero quae fuga itaque sed inventore consequatur incidunt laudantium animi tempore, necessitatibus consequuntur omnis laboriosam perspiciatis saepe quia culpa?
            Labore, repellendus totam, veritatis asperiores minima consequatur soluta in impedit libero itaque quasi incidunt similique ipsa exercitationem earum corrupti officia neque, eius sint sed maiores illum quos. Incidunt, ut cum?
            Illo eius nostrum veritatis iste voluptatum dolores corporis deleniti. Libero, ab qui consequuntur quam fuga placeat, repellat nemo illo officia culpa eum incidunt dolor vel veniam suscipit harum velit saepe.
            Quisquam architecto quod nobis maiores, doloremque non, consequuntur molestias iste incidunt officiis facere ipsa aperiam veritatis fugiat autem illo qui est at eius tempore, nostrum soluta facilis inventore. Perspiciatis, ad?
            Ipsam perspiciatis dolor voluptas neque, nihil quos, eos consectetur cumque iure quia, sunt quibusdam earum illum deserunt voluptates alias porro quae corrupti! Rem consequuntur, impedit incidunt magni expedita recusandae error.
            Sunt, eligendi blanditiis laudantium quaerat nobis doloribus accusamus, ratione officia, possimus voluptate nihil eius hic repellat laboriosam eveniet nostrum! Sit et quidem quia maiores exercitationem commodi quas voluptates quos atque.
        </div>
        <div class="bottomBtnStyle">
            <div class="whitePageBtn" @click="()=>navigateWhitePage(true)">Ios滑動時點我-白頁導頁</div>
            <div class="normalPageBtn" @click="() => navigateWhitePage(false)">Ios滑動時點我-無白頁導頁</div>
        </div>
    </div>
</template>

<script>
 export default{
    data() {
        return {
            
        }
    },
    mounted() {
        const scrollSection = document.getElementById('scrollSectionID')
        if(scrollSection) {
            scrollSection.scrollTop = 0
        }
        
        this.$nextTick(()=> {
            if(localStorage['needResetScroll']) {
                scrollSection.style.overflow = ''
                localStorage.removeItem('needResetScroll')
            }
        })
    },
    layout:'routerWhitePage',
    methods: {
        navigateWhitePage(isShowWhitePage) {
            if(isShowWhitePage) {
                this.$router.push('/testWhitePage2')
            } else {
                document.getElementById('scrollSectionID').style.overflow = 'hidden'
                localStorage['needResetScroll'] = 'yes'
                this.$nextTick(()=> {
                    this.$router.push('/testWhitePage2')
                })
            }
        }
    }
 }
</script>

<style scoped>

.bottomBtnStyle{
    position: fixed; 
    bottom: 0;
    height: 100px; 
    width: calc(100vw - 8px);
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    margin-left: -4px;
}

.bottomBtnStyle > div{
    line-height: 50px;
}

.changePageBtn{
    flex:1;
    background-color: rgb(147, 43, 138);
    color: white;
    width: 100%;
    text-align: center;
    border: 1px solid black;
    border-bottom: none;
}

.whitePageBtn{
    flex:1;
    background-color: rgb(72, 35, 69);
    color: white;
    width: 100%;
    text-align: center;
    border: 1px solid black;
    border-bottom: none;
}

.normalPageBtn{
    flex:1;
    width: 100%;
    text-align: center;
    border: 1px solid black;
}

.normalPageBtn:hover,
.whitePageBtn:hover{
    background-color: rgb(183, 147, 218);
    color: white;
}
</style>